...
const paths = {
    ...
    rootScss: 'src/assets/sass/root.scss',
...
function removeCssBlock(content, selector) {
    ...
    const selectorRegex = new RegExp(`\\s*${escapedSelector}(?:\\s+[^{,]+)?\\s*\\{`, 'g');
...
function cleanScss(filePath) {
    ...
    content = content.replace(/\/\*(?:(?!\*\/)[\s\S])*?[Dd]ark [Mm]ode(?:(?!\*\/)[\s\S])*?\*\/\s*/g, '');
    ...
    content = content.replace(/\s*transition:\s*background-color\s+0\.3s;\s*/g, '\n');
...
updateFile(paths.navJs, [
    ...
    },
    {
        ...
        pattern: /\s*darkModeToggle:\s*document\.querySelector\(CONFIG\.SELECTORS\.darkModeToggle\),?/g,
        replacement: ''
    }
]);
...
updateFile(paths.headerHtml, [
    {
        ...
        pattern: /\s*<!--Dark Mode toggle[\s\S]*?-->\s*<button[^>]*id="dark-mode-toggle"[\s\S]*?<\/button>/gi,
        replacement: ''
    }
]);

// 5. Update root.scss file description to remove dark mode mention
updateFile(paths.rootScss, [
    {
...
// 6. Update all SCSS files using the smart cleaner
